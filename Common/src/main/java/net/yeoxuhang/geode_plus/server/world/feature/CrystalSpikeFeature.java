...
public class CrystalSpikeFeature extends Feature<GeodeCrystalSpikeConfig> {
    ...
    public boolean place(@NotNull FeaturePlaceContext<GeodeCrystalSpikeConfig> context) {
        ...
        if (world.isStateAtPosition(blockPos.relative(config.crystal_direction.getDirection().getOpposite()), DripstoneUtils::isEmptyOrWaterOrLava) && world.getBlockState(blockPos).is(BlockTags.BASE_STONE_NETHER) && this.placeSpike(world, blockPos, radiusCheck, stepHeight, randomChance, trigList, config.crystal_direction.getDirection(), random)) {
