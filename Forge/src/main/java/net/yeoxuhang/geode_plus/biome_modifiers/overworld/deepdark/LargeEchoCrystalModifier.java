...
public class LargeEchoCrystalModifier implements BiomeModifier {
    private final HolderSet<PlacedFeature> features;

    public LargeEchoCrystalModifier(HolderSet<PlacedFeature> features) {
    ...
    private void biomeModificationAddPhase(Holder<Biome> biome, ModifiableBiomeInfo.BiomeInfo.Builder builder) {
        if (biome.is(TagRegistry.Biomes.HAS_ECHO_CRYSTAL) && !biome.is(TagRegistry.Biomes.BLACKLIST_BIOMES) && ServerConfigs.WORLDGEN.shouldGenerateEchoCrystal) {
    ...
    public Codec<? extends BiomeModifier> codec() {
        return LargeEchoCrystalModifier.makeCodec();
    }

    public static Codec<LargeEchoCrystalModifier> makeCodec() {
        return RecordCodecBuilder.create((config) -> {
            ...
            })).apply(config, LargeEchoCrystalModifier::new);
        });
    }
}