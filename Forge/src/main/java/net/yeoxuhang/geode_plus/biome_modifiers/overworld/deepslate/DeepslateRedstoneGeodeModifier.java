package net.yeoxuhang.geode_plus.biome_modifiers.overworld.deepslate;
...
public class DeepslateRedstoneGeodeModifier implements BiomeModifier {
    private final Holder<PlacedFeature> features;

    public DeepslateRedstoneGeodeModifier(Holder<PlacedFeature> features) {
    ...
    private void biomeModificationAddPhase(Holder<Biome> biome, ModifiableBiomeInfo.BiomeInfo.Builder builder) {
        if (biome.is(TagRegistry.Biomes.HAS_REDSTONE_GEODE) && !biome.is(TagRegistry.Biomes.BLACKLIST_BIOMES) && ServerConfigs.WORLDGEN.shouldGenerateRedstoneGeodes) {
            builder.getGenerationSettings().addFeature(GenerationStep.Decoration.UNDERGROUND_DECORATION, features);
        }
    }
    @Override
    public Codec<? extends BiomeModifier> codec() {
        return DeepslateRedstoneGeodeModifier.makeCodec();
    }

    public static Codec<DeepslateRedstoneGeodeModifier> makeCodec() {
        return RecordCodecBuilder.create((config) -> {
            ...
            })).apply(config, DeepslateRedstoneGeodeModifier::new);
        });
    }
}