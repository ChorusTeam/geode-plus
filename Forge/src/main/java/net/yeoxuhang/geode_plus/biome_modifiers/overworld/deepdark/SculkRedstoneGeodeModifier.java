package net.yeoxuhang.geode_plus.biome_modifiers.overworld.deepdark;
...
public class SculkRedstoneGeodeModifier implements BiomeModifier {
    private final Holder<PlacedFeature> features;

    public SculkRedstoneGeodeModifier(Holder<PlacedFeature> features) {
    ...
    private void biomeModificationAddPhase(Holder<Biome> biome, ModifiableBiomeInfo.BiomeInfo.Builder builder) {
        if (biome.is(TagRegistry.Biomes.HAS_ECHO_GEODE) && !biome.is(TagRegistry.Biomes.BLACKLIST_BIOMES) && ServerConfigs.WORLDGEN.shouldGenerateRedstoneGeodes) {
            builder.getGenerationSettings().addFeature(GenerationStep.Decoration.UNDERGROUND_DECORATION, features);
        }
    }
    @Override
    public Codec<? extends BiomeModifier> codec() {
        return SculkRedstoneGeodeModifier.makeCodec();
    }

    public static Codec<SculkRedstoneGeodeModifier> makeCodec() {
        return RecordCodecBuilder.create((config) -> {
            ...
            })).apply(config, SculkRedstoneGeodeModifier::new);
        });
    }
}