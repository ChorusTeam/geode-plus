...
public class SculkLapisGeodeModifier implements BiomeModifier {
    private final Holder<PlacedFeature> features;

    public SculkLapisGeodeModifier(Holder<PlacedFeature> features) {
    ...
    private void biomeModificationAddPhase(Holder<Biome> biome, ModifiableBiomeInfo.BiomeInfo.Builder builder) {
        if (biome.is(TagRegistry.Biomes.HAS_ECHO_GEODE)&& !biome.is(TagRegistry.Biomes.BLACKLIST_BIOMES) && ServerConfigs.WORLDGEN.shouldGenerateLapisGeodes) {
            builder.getGenerationSettings().addFeature(GenerationStep.Decoration.UNDERGROUND_DECORATION, features);
        }
    }
    @Override
    public Codec<? extends BiomeModifier> codec() {
        return SculkLapisGeodeModifier.makeCodec();
    }

    public static Codec<SculkLapisGeodeModifier> makeCodec() {
        return RecordCodecBuilder.create((config) -> {
            ...
            })).apply(config, SculkLapisGeodeModifier::new);
        });
    }
}