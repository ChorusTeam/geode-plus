...
public class BlackstoneGlowstoneGeodeModifier implements BiomeModifier {
    private final Holder<PlacedFeature> features;

    public BlackstoneGlowstoneGeodeModifier(Holder<PlacedFeature> features) {
    ...
    private void biomeModificationAddPhase(Holder<Biome> biome, ModifiableBiomeInfo.BiomeInfo.Builder builder) {
        if (biome.is(TagRegistry.Biomes.HAS_BLACKSTONE_GLOWSTONE_GEODE) && !biome.is(TagRegistry.Biomes.BLACKLIST_BIOMES) && ServerConfigs.WORLDGEN.shouldGenerateNetherGlowstoneGeodes){
            builder.getGenerationSettings().addFeature(GenerationStep.Decoration.UNDERGROUND_DECORATION, features);
        }
    }
    @Override
    public Codec<? extends BiomeModifier> codec() {
        return BlackstoneGlowstoneGeodeModifier.makeCodec();
    }

    public static Codec<BlackstoneGlowstoneGeodeModifier> makeCodec() {
        return RecordCodecBuilder.create((config) -> {
            ...
            })).apply(config, BlackstoneGlowstoneGeodeModifier::new);
        });
    }
}