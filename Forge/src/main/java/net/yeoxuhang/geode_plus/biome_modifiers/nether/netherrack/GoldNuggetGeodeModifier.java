...
public class GoldNuggetGeodeModifier implements BiomeModifier {
    private final Holder<PlacedFeature> features;

    public GoldNuggetGeodeModifier(Holder<PlacedFeature> features) {
    ...
    private void biomeModificationAddPhase(Holder<Biome> biome, ModifiableBiomeInfo.BiomeInfo.Builder builder) {
        if (biome.is(TagRegistry.Biomes.HAS_NETHER_GOLD_NUGGET_GEODE) && !biome.is(TagRegistry.Biomes.BLACKLIST_BIOMES) && ServerConfigs.WORLDGEN.shouldGenerateNetherGoldNuggetGeodes) {
            builder.getGenerationSettings().addFeature(GenerationStep.Decoration.UNDERGROUND_DECORATION, features);
        }
    }
    @Override
    public Codec<? extends BiomeModifier> codec() {
        return GoldNuggetGeodeModifier.makeCodec();
    }

    public static Codec<GoldNuggetGeodeModifier> makeCodec() {
        return RecordCodecBuilder.create((config) -> {
            ...
            })).apply(config, GoldNuggetGeodeModifier::new);
        });
    }
}