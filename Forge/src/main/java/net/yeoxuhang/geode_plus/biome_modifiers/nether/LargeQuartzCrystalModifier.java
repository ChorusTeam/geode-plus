...
public class LargeQuartzCrystalModifier implements BiomeModifier {
    private final HolderSet<PlacedFeature> features;

    public LargeQuartzCrystalModifier(HolderSet<PlacedFeature> features) {
    ...
    private void biomeModificationAddPhase(Holder<Biome> biome, ModifiableBiomeInfo.BiomeInfo.Builder builder) {
        if (biome.is(TagRegistry.Biomes.HAS_QUARTZ_CRYSTAL) && !biome.is(TagRegistry.Biomes.BLACKLIST_BIOMES) && ServerConfigs.WORLDGEN.shouldGenerateQuartzCrystal) {
    ...
    public Codec<? extends BiomeModifier> codec() {
        return LargeQuartzCrystalModifier.makeCodec();
    }

    public static Codec<LargeQuartzCrystalModifier> makeCodec() {
        return RecordCodecBuilder.create((config) -> {
            ...
            })).apply(config, LargeQuartzCrystalModifier::new);
        });
    }
}