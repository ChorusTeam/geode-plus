...
public class BasaltGlowstoneGeodeModifier implements BiomeModifier {
    private final Holder<PlacedFeature> features;

    public BasaltGlowstoneGeodeModifier(Holder<PlacedFeature> features) {
    ...
    private void biomeModificationAddPhase(Holder<Biome> biome, ModifiableBiomeInfo.BiomeInfo.Builder builder) {
        if (biome.is(TagRegistry.Biomes.HAS_BASALT_GLOWSTONE_GEODE) && !biome.is(TagRegistry.Biomes.BLACKLIST_BIOMES) && ServerConfigs.WORLDGEN.shouldGenerateNetherGlowstoneGeodes) {
            builder.getGenerationSettings().addFeature(GenerationStep.Decoration.UNDERGROUND_DECORATION, features);
        }
    }
    @Override
    public Codec<? extends BiomeModifier> codec() {
        return BasaltGlowstoneGeodeModifier.makeCodec();
    }

    public static Codec<BasaltGlowstoneGeodeModifier> makeCodec() {
        return RecordCodecBuilder.create((config) -> {
            ...
            })).apply(config, BasaltGlowstoneGeodeModifier::new);
        });
    }
}