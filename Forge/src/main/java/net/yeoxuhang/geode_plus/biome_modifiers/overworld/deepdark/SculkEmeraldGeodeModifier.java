...
public class SculkEmeraldGeodeModifier implements BiomeModifier {
    private final Holder<PlacedFeature> features;

    public SculkEmeraldGeodeModifier(Holder<PlacedFeature> features) {
    ...
    private void biomeModificationAddPhase(Holder<Biome> biome, ModifiableBiomeInfo.BiomeInfo.Builder builder) {
        if (biome.is(TagRegistry.Biomes.HAS_ECHO_GEODE) && !biome.is(TagRegistry.Biomes.BLACKLIST_BIOMES) && ServerConfigs.WORLDGEN.shouldGenerateEmeraldGeodes) {
            builder.getGenerationSettings().addFeature(GenerationStep.Decoration.UNDERGROUND_DECORATION, features);
        }
    }
    @Override
    public Codec<? extends BiomeModifier> codec() {
        return SculkEmeraldGeodeModifier.makeCodec();
    }

    public static Codec<SculkEmeraldGeodeModifier> makeCodec() {
        return RecordCodecBuilder.create((config) -> {
            ...
            })).apply(config, SculkEmeraldGeodeModifier::new);
        });
    }
}