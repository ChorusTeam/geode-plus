...
public class RedstoneGeodeModifier implements BiomeModifier {
    private final Holder<PlacedFeature> features;

    public RedstoneGeodeModifier(Holder<PlacedFeature> features) {
    ...
    public Codec<? extends BiomeModifier> codec() {
        return RedstoneGeodeModifier.makeCodec();
    }

    public static Codec<RedstoneGeodeModifier> makeCodec() {
        return RecordCodecBuilder.create((config) -> {
            ...
            })).apply(config, RedstoneGeodeModifier::new);
        });
    }
}