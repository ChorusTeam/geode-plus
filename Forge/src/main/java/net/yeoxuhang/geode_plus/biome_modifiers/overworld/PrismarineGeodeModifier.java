...
public class PrismarineGeodeModifier implements BiomeModifier {
    private final Holder<PlacedFeature> features;

    public PrismarineGeodeModifier(Holder<PlacedFeature> features) {
    ...
    private void biomeModificationAddPhase(Holder<Biome> biome, ModifiableBiomeInfo.BiomeInfo.Builder builder) {
        if (biome.is(TagRegistry.Biomes.HAS_PRISMARINE_GEODE) && ServerConfigs.WORLDGEN.shouldGeneratePrismarineGeodes) {
            builder.getGenerationSettings().addFeature(GenerationStep.Decoration.UNDERGROUND_DECORATION, features);
        }
    }
    @Override
    public Codec<? extends BiomeModifier> codec() {
        return PrismarineGeodeModifier.makeCodec();
    }

    public static Codec<PrismarineGeodeModifier> makeCodec() {
        return RecordCodecBuilder.create((config) -> {
            ...
            })).apply(config, PrismarineGeodeModifier::new);
        });
    }
}